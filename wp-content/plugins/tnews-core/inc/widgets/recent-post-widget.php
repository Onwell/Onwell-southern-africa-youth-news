...
class tnews_recent_posts_widget extends WP_Widget {
        ...
        function __construct() {
            ...
            parent::__construct(
                ...
                'tnews_recent_posts_widget',

                // Widget name will appear in UI

                esc_html__( 'Tnews :: Recent Posts', 'tnews' ),
                ...
                array(
                    ...
                    'description'                   => esc_html__( 'Add Recent Posts Widget', 'tnews' ),
                )
            );
        }
                            ...
                            echo '<div class="recent-post">';
                                    ...
                                    echo '<div class="media-img">';
                                        echo '<a href="'.get_the_permalink().'">';
                                            echo tnews_img_tag( array(
                                                "url"   => esc_url( get_the_post_thumbnail_url( null, 'tnews_80X80' ) ),
                                            ) );
                                        echo '</a>';
                                    echo '</div>';
                                }
                                echo '<div class="media-body">';
                                    echo '<h4 class="post-title">';
                                        echo '<a class="hover-line" href="'.esc_url( get_the_permalink() ).'">'.wp_trim_words( wp_kses_post( get_the_title() ), $post_title_count, '' ).'</a>';
                                    echo '</h4>';
                                    echo '<div class="recent-post-meta">';
                                    if($show_date){
                                        echo '<a href="'.esc_url( get_the_permalink() ).'"><i class="fal fa-calendar-days"></i>'.esc_html( get_the_date() ).'</a>';
                                    }
                                    echo '</div>';
        ...
        public function form( $instance ) {
           ...
           //Post title Count
            ...
            <p>
                <label for="<?php echo $this->get_field_id( 'post_count' ); ?>"><?php _e( 'Number of Posts to show:' ,'tnews'); ?></label>
            ...
            <p>
                <label for="<?php echo $this->get_field_id( 'post_title_count' ); ?>"><?php _e( 'Number of title word to show:' ,'tnews'); ?></label>
    ...
    } // Class tnews_recent_posts_widget ends here


    // Register and load the widget
    function tnews_recent_posts_load_widget() {
        register_widget( 'tnews_recent_posts_widget' );
    }
    add_action( 'widgets_init', 'tnews_recent_posts_load_widget' );